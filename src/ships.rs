...
pub fn generate_ships(board: &mut [[u8; 10]; 10]) {
    let mut iteration = 0;
...
fn generate_l(board: &mut [[u8; 10]; 10]) -> Result<[usize; 2], ()> {
    let [x, y] = random_point(7, 8);
    // println!("X: {x}, Y: {y}");

    if board[x][y] == 1 || board[x + 1][y] == 1 || board[x + 2][y] == 1 || board[x + 2][y + 1] == 1
    {
...
fn generate_z(board: &mut [[u8; 10]; 10]) -> Result<[usize; 2], ()> {
    let [x, y] = random_point(7, 8);
    // println!("X: {x}, Y: {y}");
...
fn generate_t(board: &mut [[u8; 10]; 10]) -> Result<[usize; 2], ()> {
...
fn generate_duo(board: &mut [[u8; 10]; 10]) -> Result<[usize; 2], ()> {
...
mod tests {
    ...
    fn create_ships() {
        for _ in 0..100 {
            let mut board = [[0u8; 10]; 10];
            generate_ships(&mut board);
            crate::print_screen(board, board);
        }
    }
}
